DROP TABLE IF EXISTS 会員,本,蔵書,貸出可能,貸出,返却,予約,予約取消,取置,読書;
...
DROP SCHEMA IF EXISTS 読書 CASCADE;
CREATE SCHEMA 読書;
...
CREATE SEQUENCE 読書.メモ番号;

CREATE TABLE 読書.読書メモ (
    メモ番号 INTEGER PRIMARY KEY,
    本のタイトル VARCHAR(100) NOT NULL,
    メモ VARCHAR(140) NOT NULL
);
...
CREATE TABLE 蔵書.取置中 (
    蔵書番号 VARCHAR(40) PRIMARY KEY REFERENCES 予約.準備完了,
    登録日時 TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
